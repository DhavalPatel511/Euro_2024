...
def plot_shots(df, team_name):
    ...
    # Separate on target and off target
    ...
    pitch.scatter(shots_on_target["x"], shots_on_target["y"], color=THEME_COLORS['success'], 
                 s=100, label="Shots On Target", edgecolors=THEME_COLORS['text'], ax=ax)
    pitch.scatter(shots_off_target["x"], shots_off_target["y"], color=THEME_COLORS['danger'], 
                 s=100, label="Shots Off Target", edgecolors=THEME_COLORS['text'], ax=ax)

    ## Adding shot count text
    pitch.text(80, 55, f"Shots On Target: {len(shots_on_target)}", 
              color=THEME_COLORS['success'], fontsize=12, ax=ax)
    pitch.text(85, 55, f"Shots Off Target: {len(shots_off_target)}", 
              color=THEME_COLORS['danger'], fontsize=12, ax=ax)
    
    ax.legend(loc="lower right", fontsize=10, facecolor=THEME_COLORS['neutral'], 
             edgecolor=THEME_COLORS['text'], title="Shot Outcomes")
    ax.set_title(f"Shots taken by {team_name}", color=THEME_COLORS['text'], fontsize=14)
...
def passes_assisted_shot(df, team_name):
    df_pass = df.loc[(df.pass_assisted_shot_id.notnull()) & (df.team_name == team_name),
                    ['x','y','end_x','end_y','pass_assisted_shot_id']]

    df_shot = (df.loc[(df.type_name == "Shot") & (df.team_name == team_name),
    ...
    pitch.lines(df_pass[mask_goal].x, df_pass[mask_goal].y, df_pass[mask_goal].end_x, df_pass[mask_goal].end_y,
               lw=8, comet=True, color=THEME_COLORS['primary'], alpha=0.8,
               label='pass leading to goal', ax=axs)

    # Plot the goals
    pitch.scatter(df_pass[mask_goal].end_x, df_pass[mask_goal].end_y, s=250,
                 marker='football', edgecolors=THEME_COLORS['dark'], c=THEME_COLORS['text'], zorder=2,
                 label='goal', ax=axs)

    # Title and legend
    axs.set_title(f"{team_name}'s passes leading to goals", color=THEME_COLORS['text'], 
                 fontsize=14, fontweight='bold')
    axs.legend(facecolor=THEME_COLORS['neutral'], edgecolor=THEME_COLORS['text'], 
...
def plot_xg_vs_goals(df, team_name):
    ...
    match_xg = team_data.groupby('match_id').agg({
        'shot_statsbomb_xg': 'sum', 
        'outcome_name': lambda x: (x == 'Goal').sum()
    }).reset_index()
    ...
    match_xg.set_index('match_id')[['Total xG', 'Actual Goals']].plot(
        kind='bar', ax=ax, color=[THEME_COLORS['primary'], THEME_COLORS['success']])
    
    ax.set_ylabel("Goals / xG", color=THEME_COLORS['text'])
    ax.set_title(f"xG vs. Actual Goals - {team_name}", color=THEME_COLORS['text'])
    ax.tick_params(colors=THEME_COLORS['text'])
    ax.spines['bottom'].set_color(THEME_COLORS['text'])
    ax.spines['top'].set_color(THEME_COLORS['text'])
    ax.spines['right'].set_color(THEME_COLORS['text'])
    ax.spines['left'].set_color(THEME_COLORS['text'])
...
def plot_possession_share(df, team_name):
    ...
    ax.set_title(f'Possession Share by Pitch Zone - {team_name}', color=THEME_COLORS['text'])
...
def shot_accuracy(df, team_name):
    ...
    shots_on_target = team_data[(team_data['type_name'] == 'Shot') & 
                               ((team_data['outcome_name'] == 'Goal') | 
                                (team_data['outcome_name'] == 'Saved'))].shape[0]
    ...
    ax.pie([shots_on_target, missed_shots], labels=["On Target", "Missed"], 
           ...
           textprops={'color': THEME_COLORS['text']})
    ax.set_title(f"Shot Accuracy - {team_name}", color=THEME_COLORS['text'])
...
def most_dangerous_attacking_players(df, team_name):
    ...
    ax.bar(goal_involvements['player_name'], goal_involvements["Total Involvements"], 
           color=THEME_COLORS['primary'])
    ax.set_title("Most Dangerous Players (Goals + Assists)", color=THEME_COLORS['text'])
    ax.set_xlabel("Player", color=THEME_COLORS['text'])
    ax.set_ylabel("Total Goal Involvements", color=THEME_COLORS['text'])
...
def create_attacker_radar(df, player1_name, player2_name):
    ...
    radar.draw_circles(ax=axs['radar'], facecolor=THEME_COLORS['light'], 
                      edgecolor=THEME_COLORS['neutral'])
    radar_poly, radar_poly2, vertices1, vertices2 = radar.draw_radar_compare(
        normalized_player1_values, normalized_player2_values, ax=axs['radar'],
        kwargs_radar={'facecolor': THEME_COLORS['primary'], 'alpha': 0.6},
    ...
    radar.draw_param_labels(ax=axs['radar'], fontsize=12, color=THEME_COLORS['text'])
...
def fouls_and_cards(df, team_name):
    ...
    yellow_fouls = team_data[(team_data["foul_committed_card_name"] == "Yellow Card") | 
                            (team_data["foul_committed_card_name"] == "Second Yellow")]
    ...
    foul_stats.plot(x="player_name", kind="bar", stacked=False, ax=ax, color=colors, 
                   edgecolor=THEME_COLORS['text'])
    ax.set_title("Fouls & Cards", color=THEME_COLORS['text'])
    ...
    ax.spines['right'].set_color(THEME_COLORS['text'])
    ax.spines['left'].set_color(THEME_COLORS['text'])
    ax.legend(["Fouls", "Yellow Cards", "Red Cards"], loc="upper right", title="Type",
             facecolor=THEME_COLORS['neutral'], edgecolor=THEME_COLORS['text'])
...
def pressing_zones(df, team_name):
    ...
    ax.scatter(low_block["x"], low_block["y"], color=THEME_COLORS['info'], 
              alpha=0.7, label="Low Block Recoveries", s=60)

    ax.text(10, 75, f"Low Blocks: {len(low_block)}", color=THEME_COLORS['info'], fontsize=10)
    ax.text(90, 75, f"High Press: {len(high_press)}", color=THEME_COLORS['danger'], fontsize=10)

    ax.legend(loc="lower center", fontsize=10, facecolor=THEME_COLORS['neutral'], 
             edgecolor=THEME_COLORS['text'], title="Ball Recoveries")
...
def duels_won_percent(df, team_name):
    ...
    aerial_lost = team_data[(team_data['type_name'] == 'Duel') & 
                           (team_data['sub_type_name'] == "Aerial Lost")].shape[0]
    ground_lost = team_data[(team_data['type_name'] == 'Duel') & 
                           (team_data['sub_type_name'] == "Tackle") & 
                           ((team_data['outcome_name'] == "Lost Out") | 
                            (team_data['outcome_name'] == "Lost In Play"))].shape[0]
    ...
    ax.pie([total_won, total_lost], labels=["Duels Won", "Duels Lost"], 
           autopct="%1.1f%%", colors=[THEME_COLORS['success'], THEME_COLORS['danger']],
           textprops={'color': THEME_COLORS['text']})
    ax.set_title(f"Duel Win Percentage - {team_name}", color=THEME_COLORS['text'])
...
def most_dangerous_defensive_players(df, team_name):
    ...
    # Interceptions count
    interceptions = team_data[(team_data['type_name'] == 'Pass') & 
                             (team_data['sub_type_name'] == 'Interception')]
    ...
    # Tackles count
    tackles = team_data[(team_data['sub_type_name'] == "Tackle") & 
                       ((team_data['outcome_name'] == 'Won') | 
                        (team_data['outcome_name'] == 'Success In Play') | 
                        (team_data['outcome_name'] == 'Success Out'))]
    tackles_cnt = tackles.groupby('player_name').size().reset_index(name='Tackles')

    # Merge defensive actions
    defensive_involvements = tackles_cnt.merge(
        interception_cnt.merge(blocks_cnt, on='player_name', how='outer'), 
        on='player_name', how='outer').fillna(0)
    defensive_involvements['Defensive Contribution'] = (defensive_involvements["Interception"] + 
                                                       defensive_involvements["Block"] + 
                                                       defensive_involvements["Tackles"])
    ...
    ax.bar(defensive_involvements['player_name'], defensive_involvements["Defensive Contribution"], 
           color=THEME_COLORS['info'])
    ax.set_title("Defensive Threats (Blocks + Tackles + Interceptions)", color=THEME_COLORS['text'])
    ax.set_xlabel("Player", color=THEME_COLORS['text'])
    ax.set_ylabel("Defensive Contribution", color=THEME_COLORS['text'])
...
def create_def_radar(df, player1_name, player2_name):
    """Create radar chart comparing two defensive players using mplsoccer"""
    # Extract player stats
    ...
    radar_poly, radar_poly2, vertices1, vertices2 = radar.draw_radar_compare(
        normalized_player1_values, normalized_player2_values, ax=axs['radar'],
        kwargs_radar={'facecolor': THEME_COLORS['info'], 'alpha': 0.6},
...
def create_gk_radar(df, player1_name, player2_name):
    ...
    # Plot radar
    radar.setup_axis(ax=axs['radar']) 
    radar.draw_circles(ax=axs['radar'], facecolor=THEME_COLORS['light'], edgecolor=THEME_COLORS['neutral'])
